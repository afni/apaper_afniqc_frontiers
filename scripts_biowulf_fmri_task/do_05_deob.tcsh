...
if ( -d /lscratch/$SLURM_JOBID ) then
    set usetemp  = 1
    set sdir_BW  = ${sdir_deob_anat}
    set sdir_deob_anat  = /lscratch/$SLURM_JOBID/${subj} #_${ses}
...
\mkdir -p ${sdir_deob_anat}

cd ${sdir_deob_anat}

set oname = `3dinfo -prefix ${dset_anat_00}`

# remove obliquity while preserving coord origin; preserve name
3dcopy  ${dset_anat_00}   tmp
3drefit -oblique_recenter tmp+orig.HEAD
3drefit -deoblique        tmp+orig.HEAD
3dcopy  tmp+orig.HEAD     ${oname}
...
if( ${usetemp} && -d ${sdir_deob_anat} ) then
    echo "++ Used /lscratch"
    echo "++ Copy from: ${sdir_deob_anat}"
    echo "          to: ${sdir_BW}"
    \mkdir -p ${sdir_BW}
    \cp -pr   ${sdir_deob_anat}/* ${sdir_BW}/.
