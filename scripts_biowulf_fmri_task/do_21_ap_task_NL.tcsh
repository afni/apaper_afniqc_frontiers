...
set dir_timing     = ${dir_inroot}/data_01_timing    # simple timing
...
set timing_files  = ( ${sdir_events}/times.{CONTROL,TASK}.txt )
set stim_classes  = ( CONTROL TASK )

# control variables
#set nt_rm         = 3      # unused here
set blur_size     = 6       # input data is 3x3x4 mm**3
set final_dxyz    = 3      
set cen_motion    = 0.3
set cen_outliers  = 0.05

# afni_check_omp respects ${OMP_NUM_THREADS}, using in regress opts
# can do in many ways (-> set to available number of CPUs):
...
#
# For the scale basis block, we use dmUBLOCK with a negative parameter for
# most appropriate duration modulation scaling;  see the online doc:
# https://afni.nimh.nih.gov/pub/dist/doc/htmldoc/statistics/deconvolve_block.html
# Here, we specifically use the parameter "-2" with it, because the mean 
# TASK event was about 2s.
#
# We note that we are *not* using the "despike" block, because this is 
# task-based processing.
# 
# Here, we do not use the following option to remove any initial time points:
#   -tcat_remove_first_trs    ${nt_rm}
# ... because we have none that we need to remove.  If we *did* include that
# option, then we might have to adjust the stimulus timing values (if they 
# weren't recorded with that removal built-in).
# 
...
afni_proc.py                                                                 \
    ...
    -regress_basis_multi      "dmUBLOCK(-2)"                                 \
