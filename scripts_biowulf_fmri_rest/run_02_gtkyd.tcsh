...
EOF
    ...
    set file_all_anat = ${gdir_gtkyd}/list_anat.txt
    set file_all_epi  = ${gdir_gtkyd}/list_epi.txt

    printf "" > ${file_all_anat}
    printf "" > ${file_all_epi}

    # count the number of dsets per modality
    set file_num_anat = ${gdir_gtkyd}/list_num_anat.txt
    set file_num_epi  = ${gdir_gtkyd}/list_num_epi.txt
    ...
    foreach subj ( ${all_subj} )
        ...
        # assumes there is only one ses dir per subj, which is true here
        set sdir_anat = ( ${dir_basic}/${subj}/ses-*/anat )
        set sdir_epi  = ( ${dir_basic}/${subj}/ses-*/func )

        set dset_anat_00  = ( ${sdir_anat}/${subj}*T1w.nii.gz )
    ...
    # set up log and run
    set log = ${cdir_log}/log_${cmd}_${grp_label}.txt

    echo "tcsh -x do_${cmd}.tcsh ${file_all_epi}  \\"         >> ${scr_swarm}
    echo "       ${file_all_anat} ${grp_label} |& tee ${log}" >> ${scr_swarm}
end
