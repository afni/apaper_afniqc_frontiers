...
set gdir_gtkyd     = ${dir_gtkyd}/${grp_label}
...
# report per data dir
\mkdir -p ${gdir_gtkyd}/anat
\mkdir -p ${gdir_gtkyd}/func

# report both individual columns, and sort+uniq ones
foreach info ( ${all_info} )
    echo "++ 3dinfo -${info} ..."

    set otxt = ${gdir_gtkyd}/anat/rep_info_${info}_su.dat
    echo "# 3dinfo -${info}" > ${otxt}
    3dinfo -${info} ${all_anat} | sort | uniq >> ${otxt}

    set otxt = ${gdir_gtkyd}/anat/rep_info_${info}_detail.dat
    echo "# 3dinfo -${info}" > ${otxt}
    3dinfo -${info} -prefix ${all_anat} >> ${otxt}

    set otxt = ${gdir_gtkyd}/func/rep_info_${info}_su.dat
    echo "# 3dinfo -${info}" > ${otxt}
    3dinfo -${info} ${all_epi} | sort | uniq >> ${otxt}

    set otxt = ${gdir_gtkyd}/func/rep_info_${info}_detail.dat
    echo "# 3dinfo -${info}" > ${otxt}
...
foreach bstat ( ${all_bstat} )
    ...
    set otxt = ${gdir_gtkyd}/anat/rep_brickstat_${bstat}_detail.dat
    ...
    set otxt_su = ${gdir_gtkyd}/anat/rep_brickstat_${bstat}_su.dat
    cat ${otxt} | awk '{print $1}' | sort -n | uniq > ${otxt_su}

    set otxt = ${gdir_gtkyd}/func/rep_brickstat_${bstat}_detail.dat
    ...
    set otxt_su = ${gdir_gtkyd}/func/rep_brickstat_${bstat}_su.dat
    cat ${otxt} | awk '{print $1}' | sort -n | uniq > ${otxt_su}
...
foreach nfield ( ${all_nfield} )
    ...
    set otxt = ${gdir_gtkyd}/anat/rep_ntool_${nfield}_su.dat
    echo "# nifti_tool -disp_hdr -field ${nfield}" > ${otxt}
    nifti_tool -disp_hdr -field ${nfield} -quiet -infiles ${all_anat} \
        | sort | uniq >> ${otxt}

    set otxt = ${gdir_gtkyd}/func/rep_ntool_${nfield}_su.dat
    echo "# nifti_tool -disp_hdr -field ${nfield}" > ${otxt}
